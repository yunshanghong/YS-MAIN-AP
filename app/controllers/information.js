...
exports.getItem = async (req, res) => {
  try {
    const { informationId } = matchedData(req)
    res.status(200).json(await db.getItem(informationId, model))
  } catch (error) {
    utils.handleError(res, error)
  }
}
...
exports.deleteItem = async (req, res) => {
  try {
    await utils.isIDGood(req.user._id)
    const { informationId } = matchedData(req)
    res.status(200).json(await db.deleteItem(informationId, model))
  } catch (error) {
    utils.handleError(res, error)
  }
}
